...
spec = do
  ...
  describe "PARSING METADATA" $ do
    ...
    describe "the key-value parser" $ do
      ...
      it "should parse multi-line key value starting in the first line" $ do
        keyvalue' "Key: line1\n line2"
        `shouldParse`
        Metadata "Key" "line1\nline2"

      it "should stop parsing when another line starts" $ do
        keyvalue' "Key1: line1\n line2\nKey2: something"
        `shouldParse`
        Metadata "Key1" "line1\nline2"
  ...
  describe "PARSING COMMENTS" $ do
    ...
    describe "the linecomment parser" $ do
      ...
      it "should parse lines starting with '#' as comments" $ do
